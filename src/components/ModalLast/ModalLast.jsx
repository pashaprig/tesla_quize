...
const ModalLast = (props) => {
  return (
    <div>
      <p className={s.description}>{props.data.data.description_end}</p>
      <p className={s.title}>Нажмите<span className='contentColor'> {props.data.data.button_text_end}</span>, чтобы зарегистрироваться на платформе Tesla</p>
      ...
      <div className={s.modalBottom}>
        ...
        <a href='https://r.oldcrm.site/tesla+pl/tesla_new/?sp=103'>{props.data.data.button_text_end}<IconCheck className={s.iconCheck} width='12' height='12' alt='Иконка Check' /></a>
      </div>
    </div>
  );
}
